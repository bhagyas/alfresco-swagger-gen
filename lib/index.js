...
require("yargs") // eslint-disable-line
    .command("ui", "Show Swagger UI after generating the file.", function (yargs) {
    yargs
        .option("destination", {
    ...
    console.log("Attempting to run docker-compose with swagger-ui...\nThis copies your Swagger YAML file to a different directory.");
    console.log("copying [" + path.resolve(argv.destination) + "] to [" + path.join(__dirname, "/target/output.yaml") + "]");
    fs.copyFile(path.resolve(argv.destination), path.join(__dirname, "../target/output.yaml"), function (err) {
        compose.upAll({ cwd: path.join(__dirname), log: true }).then(function () {
            console.log("done");
        }, function (err) {
